...
v2f vert( appdata_img v )
{
	...
	o.uv[2] = uv + float2(-_MainTex_TexelSize.x*_lineWidth, -_MainTex_TexelSize.y*_lineWidth);
	o.uv[3] = uv + float2(+_MainTex_TexelSize.x*_lineWidth, -_MainTex_TexelSize.y*_lineWidth);
	return o;
}
...
inline half CheckSame (half2 centerNormal, float centerDepth, half4 sample)
{
	...
	half isSameNormal = (diff.x + diff.y) < _detectLevel;
	// difference in depth
...
half4 frag (v2f i) : COLOR
{
	...
	half black = CheckSame(centerNormal, centerDepth, sample1)*CheckSame(centerNormal, centerDepth, sample2);
	
	half4 newColor = (1-black)*_Color;
	
	original*=black;

	return original+newColor;
}
ENDCG
	}
}
